...
const simplePdf = lines => {
  ...
  let output = '%PDF-1.4\n%PDF generated by NEXAHUB by Air Sales\n'; const offsets = [0];
...
const bookingDocumentLines = async (booking, type) => {
  ...
  const title = type === 'receipt' ? 'NEXAHUB by Air Sales - TRAVEL RECEIPT' : 'NEXAHUB by Air Sales - ELECTRONIC TICKET SUMMARY';
  ...
  lines.push('', type === 'receipt' ? 'This travel receipt is generated by NEXAHUB by Air Sales.' : 'Present this document together with the traveller passport at check-in.');
  return lines;
};
