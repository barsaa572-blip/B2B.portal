...
const showCancelFlow = (modal, booking) => {
  ...
  modal.querySelector('.calculate-cancel-flow').addEventListener('click', async event => {
    ...
    try {
      ...
      const amount = value => Number.isFinite(Number(value)) ? yen(value) : '—';
      const quote = data.quote?.amountsCny || {};
      modal.innerHTML = `<section class="booking-detail"><button class="close booking-close" type="button">&times;</button><p class="eyebrow">CANCEL TICKET</p><h2>Spring refund calculation</h2><p class="modal-copy">Calculated directly by Spring Airlines for PNR ${booking.ref}. Any no-show condition is included in this calculation.</p><section class="summary-selection"><h3>Flights to cancel</h3>${selectedLegs.map(leg => `<div><strong>${leg.route}</strong><span>${leg.flight} &middot; ${leg.time}</span></div>`).join('')}<h3>Passengers</h3><p>${passengers.join(' &middot; ')}</p></section><section class="fee-summary"><div><span>Refundable ticket amount</span><b>${amount(quote.ticketAmount)}</b></div><div><span>Refundable fare</span><b>${amount(quote.refundableFare)}</b></div><div><span>Refundable taxes</span><b>${amount(quote.refundableTaxes)}</b></div><div><span>Airline cancellation / no-show fee</span><b>− ${amount(quote.cancellationFee)}</b></div><div><span>Non-refundable amount</span><b>− ${amount(quote.nonRefundable)}</b></div><div class="fee-total"><span>Refund</span><strong>${amount(quote.refund)}</strong></div></section><p class="booking-warning">This is the Spring Airlines calculation. The actual refund is not submitted yet; the next step needs the Spring order-detail IDs.</p><div class="booking-detail-actions"><button type="button" class="secondary back-booking">Back</button><button type="button" class="primary confirm-cancel" disabled>Refund submission coming next</button></div></section>`;
    } catch (error) {
