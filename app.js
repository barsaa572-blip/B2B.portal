...
const bookingFareBreakdown = booking => {
  ...
  return `<section class="booking-fare-breakdown"><h3>Price breakdown</h3>${typeRows}<div class="booking-fare-total"><span>Original ticket total</span><strong>${quoteMnt(total)}</strong></div>${changes || refunds ? `<details class="booking-financial-history"><summary>Change &amp; refund history</summary>${changes}${refunds}</details>` : ''}</section>`;
};
...
const openBookingDetail = ref => {
  ...
  const passengerList = passengers.map((name, index) => {
    ...
    return `<details class="passenger-entry passenger-disclosure"><summary class="passenger-name-row passenger-toggle"><strong>${index + 1}. ${name}</strong><i class="passenger-chevron" aria-hidden="true"></i><b class="passenger-type">${type}</b></summary><span>${noShow ? 'No-show recorded' : 'Passenger details'}</span><div class="passenger-document"><span>Passport number</span><b>${document.documentNumber || '—'}</b><span>Date of birth</span><b>${document.dateOfBirth || '—'}</b><span>Gender</span><b>${document.gender || '—'}</b><span>Nationality</span><b>${document.nationality || '—'}</b><span>Passport expiry</span><b>${document.documentExpiry || '—'}</b></div></details>`;
  }).join('');
    ...
    const renderMonth = () => {
      ...
      calendar.querySelectorAll('.availability-day:not(:disabled)').forEach(button => button.addEventListener('click', async () => {
        ...
        try {
          ...
          if (!data.flights?.length) throw new Error(`No matching flight schedule is available for ${button.dataset.date}. Please choose another date.`);
  ...
  modal.querySelector('.calculate-change-flow').addEventListener('click', async event => {
    ...
    try {
      const response = await secureFetch(`/api/bookings/${encodeURIComponent(booking.ref)}/change-quote`, {
        method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ bgPairList: pairs, changes })
      });
      const data = await response.json();
...
const passengerForm = (type, index) => `<section class="passenger-card" data-passenger-type="${type}"><div class="passenger-card-title"><h2>${index + 1} ${type}</h2></div><div class="passenger-fields"><label>Last name (surname)<input name="last-name" required /></label><label>First name (given names)<input name="first-name" required /></label>${splitDateField('Date of birth', 'date-of-birth')}<label>Document type<select name="document-type"><option>Passport</option><option>National ID</option></select></label><label>Document number<input name="document-number" required /></label><label>Issuing country / region<input name="issuing-country" required /></label>${splitDateField('Document expiry', 'document-expiry')}<label>Nationality / region<input name="nationality" required /></label><label>Gender<select name="gender" required><option value="">Select</option><option>Male</option><option>Female</option></select></label></div></section>`;
...
const getReturnFlights = async flight => { if (!flight.departureToken) return null; const response = await secureFetch(`/api/flights?${returnSearchQuery(flight)}`); const data = await response.json(); return response.ok && data.results?.length ? { flight: data.results[0], sameAirline: data.sameAirline } : null; };
...
if (outboundDateInput.value && outboundDateInput.value < localToday) outboundDateInput.value = localToday;
if (returnDateInput.value && outboundDateInput.value && returnDateInput.value < outboundDateInput.value) returnDateInput.value = outboundDateInput.value;
outboundDateInput.addEventListener('change', () => { if (returnDateInput.value && returnDateInput.value < outboundDateInput.value) returnDateInput.value = outboundDateInput.value; });
...
const openCalendar = input => { activeDateInput = input; const [year, month] = input.value.split('-').map(Number); const fallback = new Date(calendarMinimum(input)); calendarMonth = Number.isFinite(year) && Number.isFinite(month) ? new Date(year, month - 1, 1) : new Date(fallback.getFullYear(), fallback.getMonth(), 1); const rect = input.getBoundingClientRect(); calendarPopover.style.top = `${Math.min(rect.bottom + 7, window.innerHeight - 345)}px`; calendarPopover.style.left = `${Math.min(rect.left, window.innerWidth - 320)}px`; calendarPopover.hidden = false; renderCalendar(); };
...
const commonAirports = { ULN: 'Ulaanbaatar', UBN: 'Ulaanbaatar', PVG: 'Shanghai', SHA: 'Shanghai' };
const offlineLocations = [{ city: 'Ulaanbaatar', code: 'UBN', airport: 'Chinggis Khaan International Airport' }, { city: 'Shanghai', code: 'PVG', airport: 'Pudong International Airport' }, { city: 'Shanghai', code: 'SHA', airport: 'Hongqiao International Airport' }, { city: 'Beijing', code: 'PEK', airport: 'Capital International Airport' }, { city: 'Beijing', code: 'PKX', airport: 'Daxing International Airport' }, { city: 'Hong Kong', code: 'HKG', airport: 'Hong Kong International Airport' }, { city: 'Tokyo', code: 'NRT', airport: 'Narita International Airport' }, { city: 'Seoul', code: 'ICN', airport: 'Incheon International Airport' }];
const setupAirportAutocomplete = (inputId, listId) => {
  ...
  input.value = input.value.replace(/\(ULN\)/gi, '(UBN)');
  ...
  const choose = option => { const code = option.code === 'ULN' ? 'UBN' : option.code; input.value = `${option.city} (${code})`; clear(); };
...
const loadWallet = async () => { const session = portalSession(); if (!session.accessToken) return; try { const response = await secureFetch('/api/wallet'); const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Wallet could not be loaded.'); if (portalSession().profile?.id !== session.profile?.id) return; const balance = Number(data.wallet?.balance_cny || 0); const rate = Number(data.rate?.effectiveRateMnt || pricingRate?.effectiveRateMnt || 0); const asMnt = value => rate ? mnt(Number(value || 0) * rate) : '—'; document.querySelectorAll('#wallet-balance, .balance-copy').forEach(element => { element.textContent = asMnt(balance); }); document.querySelectorAll('#wallet-balance-cny, .balance-cny-copy').forEach(element => { element.textContent = `${cny(balance)} CNY`; }); const ledger = document.querySelector('#ledger'); if (!ledger) return; let runningBalance = balance; ledger.innerHTML = (data.transactions || []).map(item => { const balanceAfter = runningBalance; runningBalance -= Number(item.amount_cny || 0); const amount = Number(item.amount_cny || 0); const sign = amount >= 0 ? '+' : '−'; return `<tr><td>${new Date(item.created_at).toLocaleString('en-GB')}</td><td>WLT-${String(item.id || '').slice(0, 8).toUpperCase()}</td><td>${escapeHtml(item.reason || 'Wallet transaction')}</td><td>${walletEntryLabel(item.entry_type)}</td><td class="${amount >= 0 ? 'credit' : 'debit'}">${sign} ${asMnt(Math.abs(amount))}<small class="currency-secondary">${sign} ${cny(Math.abs(amount))} CNY</small></td><td>${asMnt(balanceAfter)}<small class="currency-secondary">${cny(balanceAfter)} CNY</small></td></tr>`; }).join('') || '<tr><td colspan="6" class="no-bookings">No wallet transactions yet.</td></tr>'; } catch (error) { if (portalSession().profile?.id !== session.profile?.id) return; const target = document.querySelector('#ledger'); if (target) target.innerHTML = `<tr><td colspan="6" class="no-bookings">${escapeHtml(error.message || 'Unable to load transaction history.')} <button type="button" class="text-btn" data-retry-wallet>Retry</button></td></tr>`; } };
...
const loadTopupInvoices = async () => { const target = document.querySelector('#topup-invoices'); if (!target || !portalSession().accessToken) return; try { const response = await secureFetch('/api/topups'); const rows = await response.json(); if (!response.ok) throw new Error(rows.error); const invoiceTotal = item => Number(item.total_mnt ?? Number(item.amount_mnt || 0) + Number(item.service_fee_mnt || 0)); const pendingRows = rows.filter(item => item.status === 'pending'); const pending = pendingRows.reduce((sum, item) => sum + invoiceTotal(item), 0); const total = document.querySelector('#pending-topups'); if (total) total.textContent = mnt(pending); const dashboardPending = document.querySelector('#dashboard-pending'); const dashboardPendingNote = document.querySelector('#dashboard-pending-note'); if (dashboardPending) dashboardPending.textContent = mnt(pending); if (dashboardPendingNote) dashboardPendingNote.textContent = `${pendingRows.length.toLocaleString('en-US')} invoice${pendingRows.length === 1 ? '' : 's'} awaiting finance approval`; target.innerHTML = rows.map(item => { const tag = item.status === 'approved' ? 'ticketed' : item.status === 'cancelled' ? 'cancelled' : 'pending'; return `<tr><td><strong>${item.invoice_number}</strong></td><td>${new Date(item.created_at).toLocaleDateString('en-GB')}</td><td><strong>${mnt(invoiceTotal(item))}</strong><small class="currency-secondary">Wallet credit: ${cny(item.amount_cny)} CNY</small></td><td><span class="tag ${tag}">${item.status}</span></td><td><button class="text-btn view-invoice" data-invoice-id="${item.id}" data-invoice-number="${item.invoice_number}">View</button><button class="text-btn download-invoice" data-invoice-id="${item.id}" data-invoice-number="${item.invoice_number}">Download</button>${item.status === 'pending' ? `<button class="text-btn delete-invoice" data-invoice-id="${item.id}">Delete</button>` : ''}</td></tr>`; }).join('') || '<tr><td colspan="5" class="no-bookings">No top-up invoices yet.</td></tr>'; } catch (error) { target.innerHTML = `<tr><td colspan="5" class="no-bookings">${error.message || 'Unable to load invoices.'}</td></tr>`; } };
...
const loadDashboard = async () => {
  ...
  try {
    ...
    document.querySelector('#dashboard-pending').textContent = mnt(data.pendingTopupMnt);
    ...
    document.querySelector('#dashboard-pending-note').textContent = `${Number(data.pendingTopupRequests).toLocaleString('en-US')} invoice${Number(data.pendingTopupRequests) === 1 ? '' : 's'} awaiting finance approval`;
  } catch {
