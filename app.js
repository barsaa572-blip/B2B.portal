...
const fareSelectionFlight = (flight, label, phase) => `<article class="selection-card fare-selection-card"><div class="selection-label">${label}</div><div class="selection-main"><div><strong>${flight?.airline || 'Airline'}</strong><span>${flight?.number || 'Flight'} · ${(flight?.fare?.cabin || flight?.fare?.fareType || 'Economy')}</span></div><div class="fare-selection-route"><strong>${flight?.departure?.id || ''} → ${flight?.arrival?.id || ''}</strong><span>${fareSelectionDate(phase)} · ${(flight?.departure?.time || '').slice(-5)} – ${(flight?.arrival?.time || '').slice(-5)}</span></div><div class="selection-duration">${formatMinutes(flight?.duration || 0)} · ${flight?.stops ? `${flight.stops} stop${flight.stops > 1 ? 's' : ''}` : 'Nonstop'}</div></div></article>`;
