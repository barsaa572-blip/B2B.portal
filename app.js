...
const passengerForm = (type, index) => `<section class="passenger-card" data-passenger-type="${type}"><div class="passenger-card-title"><h2>${index + 1} ${type}</h2></div><div class="passenger-fields"><label>Last name (surname)<input name="last-name" required /></label><label>First name (given names)<input name="first-name" required /></label>${splitDateField('Date of birth', 'date-of-birth')}<label>Document type<select name="document-type"><option>Passport</option><option>National ID</option></select></label><label>Document number<input name="document-number" required /></label><label>Issuing country / region<input name="issuing-country" required placeholder="e.g. Mongolia" /></label>${splitDateField('Document expiry', 'document-expiry')}<label>Nationality / region<input name="nationality" required placeholder="e.g. Mongolia" /></label><label>Gender<select name="gender" required><option value="">Select</option><option>Male</option><option>Female</option></select></label></div></section>`;
