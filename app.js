...
const openBookingScreen = () => { document.body.classList.add('booking-mode'); const checkout = resultArea.querySelector('.passenger-checkout'); if (!checkout || checkout.querySelector('.booking-route')) return; const route = `${selectedOutbound?.departure?.id || ''} <i>↔</i> ${selectedOutbound?.arrival?.id || ''}`; checkout.insertAdjacentHTML('afterbegin', `<div class="booking-route"><span>${selectedReturn ? 'Round trip' : 'One way'}</span><h1>${route}</h1></div>`); checkout.querySelector('.checkout-heading').insertAdjacentHTML('afterend', `<div class="booking-service"><strong>Booking code selected · Economy</strong><button class="docs-import" type="button">DOCS import</button></div>`); };
