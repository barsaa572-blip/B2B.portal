...
const fareSelectionFlight = (flight, label) => `<article class="fare-selected-flight"><span class="fare-selected-label">${label}</span><div class="fare-selected-carrier"><i>${airlineLogo(flight?.airline, flight?.airlineLogo, `${flight?.airline || 'Airline'} logo`)}</i><span><b>${flight?.airline || 'Airline'}</b><small>${flight?.number || 'Flight'} · ${(flight?.fare?.cabin || flight?.fare?.fareType || 'Economy')}</small></span></div><div class="fare-selected-time"><b>${(flight?.departure?.time || '').slice(-5)} ${flight?.departure?.id || ''}</b><small>${flight?.departure?.name || ''}</small></div><div class="fare-selected-duration"><b>${formatMinutes(flight?.duration || 0)}</b><i></i><small>${flight?.stops ? `${flight.stops} stop${flight.stops > 1 ? 's' : ''}` : 'Nonstop'}</small></div><div class="fare-selected-time"><b>${(flight?.arrival?.time || '').slice(-5)} ${flight?.arrival?.id || ''}</b><small>${flight?.arrival?.name || ''}</small></div></article>`;
