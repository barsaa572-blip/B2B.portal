(() => {
  ...
  const openAgency = () => {
    const element = modal();
    element.innerHTML = `<form class="admin-form"><button type="button" class="close">×</button><p class="eyebrow">NEW AGENCY</p><h2>Create agency account</h2><p>Creates an isolated wallet and booking workspace.</p><label>Agency name<input name="name" required /></label><label>Registration number<input name="registrationNumber" required /></label><label>Email address<input name="email" type="email" required /></label><label>Contact phone<input name="phone" type="tel" required /></label><label>Office address<input name="address" required /></label><label>Main office / branch<input name="branchName" /></label><label>Opening balance (MNT)<input name="initialBalanceMnt" type="number" step="1" min="0" value="0" /></label><p class="admin-form-error" hidden></p><button class="primary full">Create agency</button></form>`;
