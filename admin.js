(() => {
  ...
  const openAgency = () => {
    const element = modal();
    element.innerHTML = `<form class="admin-form"><button type="button" class="close">×</button><p class="eyebrow">NEW AGENCY</p><h2>Create agency account</h2><p>Creates an isolated wallet and booking workspace.</p><label>Agency name<input name="name" required placeholder="e.g. Airmarket Travel" /></label><label>Registration number<input name="registrationNumber" required placeholder="e.g. 1234567" /></label><label>Email address<input name="email" type="email" required placeholder="e.g. office@agency.mn" /></label><label>Contact phone<input name="phone" type="tel" required placeholder="e.g. +976 8011 5627" /></label><label>Office address<input name="address" required placeholder="e.g. Ulaanbaatar, Sukhbaatar district" /></label><label>Main office / branch<input name="branchName" placeholder="e.g. Main office" /></label><label>Opening balance (MNT)<input name="initialBalanceMnt" type="number" step="1" min="0" value="0" /></label><p class="admin-form-error" hidden></p><button class="primary full">Create agency</button></form>`;
