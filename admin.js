(() => {
  ...
  const openEditAgency = agencyId => {
    ...
    element.showModal(); element.querySelector('.close').onclick = () => closeModal(element);
    element.querySelector('form').onsubmit = async event => { event.preventDefault(); const form = event.currentTarget; const error = form.querySelector('.admin-form-error'); const values = new FormData(form); try { await api(`/api/admin/agencies/${agencyId}`, { method: 'PATCH', body: JSON.stringify({ name: values.get('name'), registrationNumber: values.get('registrationNumber'), email: values.get('email'), phone: values.get('phone'), active: form.elements.active.checked }) }); closeModal(element); await load(); notify('Agency updated.'); } catch (issue) { error.textContent = issue.message; error.hidden = false; } };
  };
